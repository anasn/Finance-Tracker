...
export function AppContent() {
  ...
  const handleSaveStock = () => {
    ...
    optimisticSave(
      () => {
        if (isEdit) {
          setStockRecords(rs => rs.map(r => r.id === tempId ? { ...r, ...body } : r));
        } else {
          setStockRecords(rs => [newRecord, ...rs]);
          if (body.paidAmount > 0) {
            const pm = body.paymentMethod || 'Cash';
            const newPayment = { id: `p_${tempId}`, userId: user?.id || '', customerId: body.customerId, stockRecordId: tempId, amount: body.paidAmount, paymentMethod: pm, bankName: body.bankName || '', transactionNote: `Stock Sale: ${body.itemName || ''}`, date: body.date || new Date().toISOString().split('T')[0], createdAt: new Date().toISOString(), customer: newRecord.customer };
            setPayments(ps => [newPayment, ...ps] as any);
            if (pm !== 'Cash' && body.bankName) {
              const newBank = { id: `b_${tempId}`, userId: user?.id || '', customerId: body.customerId, paymentDate: body.date || new Date().toISOString().split('T')[0], paymentAmount: body.paidAmount, bankName: body.bankName, accountType: pm, paymentMethod: pm, transactionNote: `Stock Sale: ${body.itemName || ''}`, createdAt: new Date().toISOString(), customer: newRecord.customer };
              setBankPayments(bs => [newBank, ...bs] as any);
            }
          }
          const invoiceDate = body.date || new Date().toISOString().split('T')[0];
          const rem = Math.max(0, body.totalAmount - body.paidAmount);
          const newInvoice = { id: `i_${tempId}`, invoiceNumber: `INV-${Date.now()}`, type: 'sale', referenceId: tempId, partyName: newRecord.customer?.name || '', partyPhone: newRecord.customer?.phone || '', partyCity: '', itemName: body.itemName, itemCategory: body.itemCategory, weight: body.weight, weightUnit: body.weightUnit, pricePerUnit: body.pricePerUnit, totalAmount: body.totalAmount, paidAmount: body.paidAmount, remainingAmount: rem, status: rem <= 0 ? 'paid' : body.paidAmount > 0 ? 'partial' : 'unpaid', notes: body.notes || '', date: invoiceDate, createdAt: new Date().toISOString() };
          setInvoices(invs => [newInvoice, ...invs] as any);
        }
      },
  ...
  const handleSavePayment = async () => {
    ...
    optimisticSave(
      () => {
        if (isEdit) {
            setPayments(ps => ps.map(p => p.id === tempId ? { ...p, ...newRecord } : p));
        } else {
            setPayments(ps => [newRecord, ...ps]);
            const invoiceDate = paymentForm.date || new Date().toISOString().split('T')[0];
            const newInvoice = { id: `i_${tempId}`, invoiceNumber: `REC-${Date.now().toString(36).toUpperCase()}`, type: 'payment', referenceId: tempId, partyName: customer?.name || '', partyPhone: customer?.phone || '', partyCity: customer?.city || '', itemName: paymentForm.transactionNote || 'Payment Received', itemCategory: paymentForm.paymentMethod || 'Cash', weight: 0, weightUnit: 'KG', pricePerUnit: 0, totalAmount: newRecord.amount, paidAmount: newRecord.amount, remainingAmount: 0, status: 'paid', notes: '', date: invoiceDate, createdAt: new Date().toISOString() };
            setInvoices(invs => [newInvoice, ...invs] as any);
        }
      },
  ...
  const handleSaveExpense = () => {
    ...
    optimisticSave(
      () => {
        if (isEdit) {
            setExpenses(es => es.map(e => e.id === tempId ? { ...e, ...newRecord } : e));
        } else {
            setExpenses(es => [newRecord, ...es]);
            const invoiceDate = expenseForm.date || new Date().toISOString().split('T')[0];
            const newInvoice = { id: `i_${tempId}`, invoiceNumber: `EXP-${Date.now().toString(36).toUpperCase()}`, type: 'expense', referenceId: tempId, partyName: expenseForm.category || 'Expense', partyPhone: '', partyCity: '', itemName: expenseForm.description || '', itemCategory: expenseForm.category || '', weight: 0, weightUnit: 'KG', pricePerUnit: 0, totalAmount: newRecord.amount, paidAmount: newRecord.amount, remainingAmount: 0, status: 'paid', notes: expenseForm.notes || '', date: invoiceDate, createdAt: new Date().toISOString() };
            setInvoices(invs => [newInvoice, ...invs] as any);
        }
      },
  ...
  const handleSavePurchase = () => {
    ...
    optimisticSave(
      () => {
        if (isEdit) {
          setPurchases(ps => ps.map(p => p.id === tempId ? { ...p, ...body } : p));
        } else {
          setPurchases(ps => [newRecord, ...ps]);
          if (body.paidAmount > 0) {
            const pm = body.paymentMethod || 'Cash';
            const newExpense = { id: `e_${tempId}`, userId: user?.id || '', description: `Purchase Payment: ${body.supplierName} - ${body.itemName}`, amount: body.paidAmount, category: 'Purchase Material', date: body.date || new Date().toISOString().split('T')[0], notes: '', createdAt: new Date().toISOString() };
            setExpenses(es => [newExpense, ...es] as any);
            if (pm !== 'Cash' && body.bankName) {
              const newBank = { id: `b_${tempId}`, userId: user?.id || '', customerId: '', paymentDate: body.date || new Date().toISOString().split('T')[0], paymentAmount: body.paidAmount, bankName: body.bankName, accountType: pm, paymentMethod: pm, transactionNote: `Purchase Payment: ${body.supplierName} - ${body.itemName}`, createdAt: new Date().toISOString() };
              setBankPayments(bs => [newBank, ...bs] as any);
            }
          }
          const invoiceDate = body.date || new Date().toISOString().split('T')[0];
          const rem = Math.max(0, body.totalAmount - body.paidAmount);
          const newInvoice = { id: `i_${tempId}`, invoiceNumber: `PINV-${Date.now()}`, type: 'purchase', referenceId: tempId, partyName: body.supplierName, partyPhone: body.supplierPhone || '', partyCity: body.supplierCity || '', itemName: body.itemName, itemCategory: body.itemCategory, weight: body.weight, weightUnit: body.weightUnit, pricePerUnit: body.pricePerUnit, totalAmount: body.totalAmount, paidAmount: body.paidAmount, remainingAmount: rem, status: rem <= 0 ? 'paid' : body.paidAmount > 0 ? 'partial' : 'unpaid', notes: body.notes || '', date: invoiceDate, createdAt: new Date().toISOString() };
          setInvoices(invs => [newInvoice, ...invs] as any);
        }
      },
  ...
  const handleInvoicePrint = () => {
    try {
      ...
      const typeLabel = inv.type === 'sale' ? 'Sale' : inv.type === 'purchase' ? 'Purchase' : inv.type === 'payment' ? 'Receipt' : 'Expense';
...
<body>
  <div class="header">
    ...
    <div class="badges">
      <span class="badge ${inv.type === 'sale' || inv.type === 'payment' ? 'badge-green' : inv.type === 'expense' ? 'badge-red' : 'badge-orange'}">${typeLabel}</span>
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground font-sans" dir={language === 'ur' ? 'rtl' : 'ltr'}>
            ...
            <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              {activeTab === 'invoices' && (() => {
                const filteredInvoices = invoices.filter(inv => {
                  const matchType = filterTypeInvoice === 'All' ? true : inv.type === filterTypeInvoice;
                  const matchDate = (!invoiceDateFrom || inv.date >= invoiceDateFrom) && (!invoiceDateTo || inv.date <= invoiceDateTo);
                  const searchStr = searchQueryInvoice.toLowerCase();
                  const matchSearch = inv.invoiceNumber.toLowerCase().includes(searchStr) || (inv.partyName || '').toLowerCase().includes(searchStr) || (inv.itemName || '').toLowerCase().includes(searchStr);
                ...
                <div className="space-y-4">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    ...
                    <div className="flex gap-1.5 bg-gray-100 dark:bg-gray-800/80 p-1.5 rounded-xl overflow-x-auto w-full md:w-auto">
                      {['All', 'sale', 'purchase', 'payment', 'expense'].map(f => (
                  ...
                  <div className="flex flex-col md:flex-row gap-3">
                    <Input placeholder={t('searchAnything')} className="h-12 text-base flex-1" value={searchQueryInvoice} onChange={e => setSearchQueryInvoice(e.target.value)} />
                    <div className="flex gap-2">
                       <Input type="date" className="h-12 w-40" value={invoiceDateFrom} onChange={e => setInvoiceDateFrom(e.target.value)} />
                       <Input type="date" className="h-12 w-40" value={invoiceDateTo} onChange={e => setInvoiceDateTo(e.target.value)} />
                       {(invoiceDateFrom || invoiceDateTo) && <Button variant="ghost" onClick={() => { setInvoiceDateFrom(''); setInvoiceDateTo(''); }} className="h-12 px-3 text-red-500 hover:bg-red-50">{t('clear')}</Button>}
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3">
                    {filteredInvoices.map(inv => (
                      <Card key={inv.id} className="shadow-sm"><CardContent className="p-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1"><h3 className="font-bold">{inv.invoiceNumber}</h3><Badge className={inv.type === 'sale' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300' : inv.type === 'payment' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : inv.type === 'purchase' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'}>{inv.type === 'sale' ? t('sale') : inv.type === 'purchase' ? t('purchase') : inv.type === 'payment' ? 'Receipt' : 'Expense'}</Badge><Badge className={inv.status === 'paid' ? 'bg-green-100 text-green-700' : inv.status === 'partial' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}>{inv.status === 'paid' ? t('paid') : inv.status === 'partial' ? t('partial') : t('unpaid')}</Badge></div>
                    ...
                    {filteredInvoices.length === 0 && <Card className="shadow-sm"><CardContent className="p-12 text-center text-gray-400"><FileText className="h-12 w-12 mx-auto mb-3 opacity-50" /><p className="text-lg">{t('noInvoicesFound')}</p></CardContent></Card>}
                  </div>
                </div>
                );
              })()}
      ...
      <Dialog open={showExpenseDialog} onOpenChange={setShowExpenseDialog}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editingExpense ? t('editExpense') : t('addNewExpense')}</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div><Label className="text-base">{t('amount')} (PKR) *</Label><Input type="number" className="h-12 text-base mt-1" placeholder="e.g. 1500" value={expenseForm.amount} onChange={e => setExpenseForm({ ...expenseForm, amount: e.target.value })} /></div>
          <div><Label className="text-base">{language === 'ur' ? 'خرچے کی تفصیل (کیا خرچہ ہوا؟) *' : 'Expense Details / Reason *'}</Label><Input className="h-12 text-base mt-1" placeholder={language === 'ur' ? "مثال: دکان کا کرایہ، چائے پانی..." : "e.g. Shop rent, Tea..."} value={expenseForm.description} onChange={e => setExpenseForm({ ...expenseForm, description: e.target.value })} /></div>
          ...
          <div><Label className="text-base">{t('notes')}</Label><Textarea className="mt-1" placeholder={language === 'ur' ? 'مزید تفصیل (اگر کوئی ہے)' : 'Additional notes (optional)'} value={expenseForm.notes} onChange={e => setExpenseForm({ ...expenseForm, notes: e.target.value })} /></div>
        ...
        {selectedInvoice && (
          <div id="invoice-print" className="space-y-6">
            <div className="text-center border-b pb-4">
              ...
              <div className="flex justify-center gap-3 mt-2"><Badge className={selectedInvoice.type === 'sale' ? 'bg-emerald-100 text-emerald-700' : selectedInvoice.type === 'payment' ? 'bg-green-100 text-green-700' : selectedInvoice.type === 'purchase' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}>{selectedInvoice.type === 'sale' ? t('sale') : selectedInvoice.type === 'purchase' ? t('purchase') : selectedInvoice.type === 'payment' ? 'Receipt' : 'Expense'}</Badge><Badge className={selectedInvoice.status === 'paid' ? 'bg-green-100 text-green-700' : selectedInvoice.status === 'partial' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}>{selectedInvoice.status === 'paid' ? t('paid') : selectedInvoice.status === 'partial' ? t('partial') : t('unpaid')}</Badge></div>
            </div>
